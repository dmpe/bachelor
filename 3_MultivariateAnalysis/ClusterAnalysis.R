...
# How many clusters ? My choice of 2 nc <- NbClust(joinedDB.6, distance = 'euclidean', method='single', index='all') nc <-
# NbClust(joinedDB.6, distance = 'euclidean', method='kmeans', index='all') NbClust(joinedDB.6, distance = 'euclidean',
...
# http://www.r-statistics.com/2013/08/k-means-clustering-from-r-in-action/ Not possible in my case, because of non-existent
...
# http://www.statmethods.net/advstats/cluster.html dfa <- scale(joinedDB.6) pamk(dfa) wss <-
# (nrow(dfa)-1)*sum(apply(dfa,2,var)) for (i in 2:15) { wss[i] <- sum(kmeans(dfa,centers=i)$withinss) } plot(1:15, wss,
...
agn
# plot(agn) plot(as.dendrogram(agn, hang = -1))


# Hierarchical Clustering http://rpubs.com/gaston/dendrograms
...
# Silhouette plot http://www.r-bloggers.com/setting-graph-margins-in-r-using-the-par-function-and-lots-of-cow-milk/
...
# sk2 <- silhouette(klust$cl, dist(joinedDB.6, method = 'euclidean')) plot(sk2)
par(mar = c(5, 10, 3, 2) + 0.1)
sk3 <- silhouette(pam(joinedDB.6, 2))
plot(sk3, max.strlen = 30)

# Who is in, who is out ?  sort(table(klust$clust))
clust <- names(sort(table(klust$clust)))
# clust row.names(mydata[klust$clust==clust[1],]) row.names(mydata[klust$clust==clust[2],])
...
ggplot(test_data_long, aes(x = vars, y = value, group = variable, color = variable)) + geom_line() + geom_point() + coord_cartesian(ylim = c(-1.2, 
    1)) + scale_y_continuous(breaks = seq(-1.2, 1, 0.25)) + theme_gdocs() + ggtitle("Means plot for clusters") + scale_color_gdocs() + 
    ylab("Mean") + xlab("Indicators") + labs(color = "Types of Countries")
