...
# GENERALLy, wrong. Because Predict with case deletion summary(joinedDB.5)
lm <- lm(LearningCurve_Index ~ Freedom_Index + WEF_Score + Ranking_HIndex + CompletionRate + Unemployment, data = joinedDB.5[2:7])
# summary(lm)

# p <- qplot(Ranking_LearningCurve, Ranking_HIndex, data = joinedDB.5) p + geom_abline(intercept = 40502.2572 , slope =
# -767.5146 ) coef(lm(joinedDB.5$Ranking_HIndex ~ joinedDB.5$Ranking_LearningCurve))
s.orig <- coef(summary(lm))[, 1:2]  # original results
s.orig

# MI mp.plot(joinedDB.5)
